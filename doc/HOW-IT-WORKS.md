...
This is what [`README.md`](../README.md) used to be. It was moved here on 2026-08-29 so
the README could lead with **where this architecture fits**; the machine itself is
described here, unchanged.

*[En español](COMO-FUNCIONA.md)*
...
![Dream projects trajectories toward bugs; the same person meets them hours later on screen](assets/night.png)
...
as conjecture ([ADR-004](adr/ADR-004-ideacion-y-proyeccion.md)). If that boundary
...
Matías's decision ([ADR-007](adr/ADR-007-la-escena-antes-del-diagrama.md)), not by
verdict.
...
"notes + sleep": it runs on top ([ADR-001](adr/ADR-001-no-competir-con-auto-dream.md)).
...
| B | Discarded alternatives kept **with the precondition** that made them right | yes ([ADR-005](adr/ADR-005-contraste-entre-implementaciones.md)) |
...
[`doc/LOGBOOK.md`](LOGBOOK.md).

## Where to look next

- [`doc/00-spec.md`](00-spec.md) — the spec
- [`doc/PLAN-TRES-IDEAS.md`](PLAN-TRES-IDEAS.md) — what each of the three ideas is still missing
- [`doc/LOGBOOK.md`](LOGBOOK.md) — what actually happened, at its real size
...
- [`doc/adr/`](adr/) — the decisions and what each one cost
