...
| El resumen de un paso es lo que el agente va a leer | Se guardaba el `tool_response` crudo, así que el presupuesto de caracteres se iba en andamiaje: `{"stdout": "…", "isImage": false, "noOutputExpected": false}` y la salida cortada por el límite. Ahora se extrae el texto que importa, con las formas verificadas de Bash y Read. **Las formas de otras tools no están verificadas**: hay un fallback que busca el primer valor con texto, y no se sabe qué tan bien anda con tools de MCP. |
